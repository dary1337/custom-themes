...
function display(array, style) {
	array.forEach((el) => document.getElementById(el) && (document.getElementById(el).style.display = style));
}
		...
		document.querySelector('container').insertAdjacentHTML(
			...
			`<div class="switch-container">
                    <div class="switch-label">
                         ...
                         <label class="dimmed">${
								(setting.id === 'backgroundUpdate' &&
									setting.description.replace(
										'%%lastUpdate%%',
										extensionSettings['lastThemesUpdate']
											? new Date(extensionSettings['lastThemesUpdate']).toLocaleString()
											: dict.strings.never
									)) ||
								setting.description ||
								''
...
const page = async (check_href = true, newTab = undefined) => {
	try {
		...
		const path = window.location.href.split('/').pop().split('#').pop().replace('index.html', '').split('-');
                         ...
                         <main id="tab" class="noSelect">
                              ...
                              <div id="tabs">
                                   <tab>
                                        ...
                                        ${
											Object.entries(userSettings).filter(([key, value]) => value.local)
												.length !== 0 || isFullPage
												? `
                                                  <div tabName="Local" 
		...
		try {
					...
					if (isFullPage) {
						...
						const openTheme = (append_href = true) => {
                                        ...
                                        <div id="theme-container">
                                             ...
                                             <div id="theme-info">
                                                       ...
                                                       <a>
                                                            ${
																userSettings[theme.id]?.edited || theme.local
																	? dict.strings.local
																	: userSettings[theme.id]?.version || theme.version
															}
                                                       </a>
                                                  </div>
                                                  <div id="theme-info-panel">
                                                       ...
                                                       ${
															!userSettings[theme.id]?.edited &&
															repos["Author's"].findIndex((x) => x.id === theme.id) !== -1
																? `
                                                                      <a 
                                                                           href="${links.newIssue}"
                                                                           target="_blank" 
							...
							if (theme.local) {
								...
								document.getElementById('export-theme').onclick = () => exportTheme(theme.id);

								for (const site of ['github', 'telegram'])
									document.getElementById('suggest-theme-' + site).onclick = (e) => {
										e.preventDefault();
										copyToClipboard(
											`Hi, I would like to suggest a theme\nTitle: ${
												userSettings[theme.id].name
											}\nWebsite: ${userSettings[theme.id].link}\n\`\`\`css\n${
												userSettings[theme.id].sourceCSS
											}\`\`\``,
											() => {
												alert(dict.strings.copiedPreset);

												openLink(site === 'github' ? links.newIssue : links.telegram);
											}
										);
									};
							}
							...
							document
								...
								.forEach((img) => (img.onclick = () => showModal(img, img.outerHTML)));

							const updateBtn = document.getElementById('theme-update-button');
							if (updateBtn)
								updateBtn.onclick = async () => {
									await updateTheme(userSettings, theme, userSettings[theme.id]?.checked);
									await page();
								};
						...
						async function saveCode() {
							...
							if (theme.local) {
								...
								if (link && !link.includes('https://') && link !== '*') link = 'https://' + link;
						...
						async function restoreTheme() {
							...
							loading_div.insertAdjacentHTML('afterbegin', getLoader(dict.loader.restoringTheme));
						...
						const openEditor = async (append_href = true) => {
							...
							document.getElementById('theme-container').innerHTML = getLoader(dict.loader.loadingCode);
							...
							if (theme.local) {
								...
								document.getElementById('theme-website').addEventListener('keydown', keyDownHandler);

								document.getElementById('theme-edit-input').addEventListener('keydown', keyDownHandler);
							}
						...
						const closeEditor = () => {
							display(['backFromThemeEditing', 'theme-h1', 'theme-edit-name'], 'none');
							display(['theme-h1'], '');
							openTheme();
							window.location.href = window.location.href.replace('-edit', '');
						};

						document.querySelector(`[slider-id="${theme.id}"]`).onclick = (e) => e.stopPropagation();
			...
			if (check_href && !href.includes('-') && themes.findIndex((theme) => href.includes(theme.id)) === -1) {
				document.querySelectorAll('.switch-container.selected').forEach((x) => x.classList.remove('selected'));
				bigTab.innerHTML = elements.emptyBigTab;
			}
